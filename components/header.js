  ...
  return (
      ...
      <header className={classNames(scrolled ? 'shadow-xl opacity-90' : 'false opacity-100','w-full backdrop-filter backdrop-blur-lg bg-gradient-to-r from-purple-900 via-sky-900 to-black background-animate fixed z-10 transition-[shadow,opacity] duration-500 ease-in-out')}>
        <div className='max-w-full mx-auto'>
          <div className='justify-center flex max-w-screen-xl false mx-auto items-center px-8 transition-p ease-in-out duration-500 '>
              ...
              <ul className="flex flex-col rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:font-medium md:bg-transparent ">
                {navigation.map((item) => (
                  <li key={item.name}>
                    ...
                    {item.href === checkSlug ? (
                      <div className='border-b-2 border-primary pb-4 text-white hover:border-primary hover:text-primary'>
                        {item.name}
                      </div>
                    ) : (
                      <div className={classNames(scrolled ? 'py-1 md:py-2' : 'py-2 md:py-4','hover:border-b-2 text-gray-300 md:text-xl lg:text-2xl hover:border-primary hover:text-primary transition-py duration-500 ease-in-out')}>{item.name}</div>
                    )}
                  </a>
